...
import { LayoutDashboard, Upload, Clock, CheckSquare, History, X, ChevronLeft, ChevronRight } from 'lucide-react';
...
const navItems = [
  ...
  { label: 'Upload Syllabus', icon: <Upload className="h-5 w-5" />, path: '/faculty/upload' },
  { label: 'Track Syllabus', icon: <Clock className="h-5 w-5" />, path: '/faculty/track' },
  { label: 'Status', icon: <CheckSquare className="h-5 w-5" />, path: '/faculty/status' },
  { label: 'History', icon: <History className="h-5 w-5" />, path: '/faculty/history' },
];

export function FacultySidebar({ isOpen, onClose, isCollapsed, onToggleCollapse }: FacultySidebarProps) {
  const { user } = useAuth();
